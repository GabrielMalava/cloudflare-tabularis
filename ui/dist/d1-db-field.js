var __tabularis_plugin__ = (function (jsx, api) {
  "use strict";
  var DRIVER = "tubularis-d1";

  var labelStyle = {
    fontSize: "10px",
    textTransform: "uppercase",
    fontWeight: 600,
    letterSpacing: "0.05em",
    color: "var(--color-text-muted, #94a3b8)",
  };

  var inputStyle = {
    width: "100%",
    padding: "7px 10px",
    background: "var(--color-bg-base, #131929)",
    border: "1px solid rgba(255,255,255,0.15)",
    borderRadius: "6px",
    color: "var(--color-text-primary, #e2e8f0)",
    fontSize: "13px",
    outline: "none",
    boxSizing: "border-box",
  };

  function field(label, value, onChange, placeholder, type) {
    return jsx.jsxs("div", {
      style: { display: "flex", flexDirection: "column", gap: "4px" },
      children: [
        jsx.jsx("label", { style: labelStyle, children: label }),
        jsx.jsx("input", {
          type: type || "text",
          value: value,
          onChange: function (e) {
            onChange(e.target.value);
          },
          autoCorrect: "off",
          autoCapitalize: "off",
          autoComplete: "off",
          spellCheck: false,
          placeholder: placeholder,
          style: inputStyle,
        }),
      ],
    });
  }

  return api
    .defineSlot("connection-modal.connection_content", function (props) {
      var ctx = props.context;
      if (ctx.driver !== DRIVER) return null;

      var accountId = typeof ctx.host === "string" ? ctx.host : "";
      var token = typeof ctx.password === "string" ? ctx.password : "";
      var database = typeof ctx.database === "string" ? ctx.database : "";

      var onAccountIdChange = ctx.onHostChange || function () {};
      var onTokenChange = ctx.onPasswordChange || function () {};
      var onDatabaseChange = ctx.onDatabaseChange || function () {};

      return jsx.jsxs("div", {
        style: { display: "flex", flexDirection: "column", gap: "12px" },
        children: [
          field(
            "Account ID",
            accountId,
            onAccountIdChange,
            "Cloudflare Account ID (leave blank to use plugin Settings)",
          ),
          field(
            "API Token",
            token,
            onTokenChange,
            "D1 Edit token (leave blank to use plugin Settings)",
            "password",
          ),
          field(
            "D1 Database (name or ID)",
            database,
            onDatabaseChange,
            "e.g. my-database or a database UUID",
          ),
          jsx.jsx("p", {
            style: {
              fontSize: "11px",
              color: "var(--color-text-muted, #94a3b8)",
              marginTop: "2px",
            },
            children:
              "Account ID and API token are per-connection. Leave them blank to fall back to this plugin's global Settings (gear icon).",
          }),
        ],
      });
    })
    .component;
})(ReactJSXRuntime, __TABULARIS_API__);
